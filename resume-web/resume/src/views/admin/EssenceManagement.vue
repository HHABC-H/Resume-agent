<template>
  <div class="essence-management">
    <div class="page-header">
      <h2>精华帖管理</h2>
      <div class="header-actions">
        <button @click="loadEssences" class="btn-refresh">刷新</button>
      </div>
    </div>

    <div class="stats-cards">
      <div class="stat-card">
        <span class="stat-value">{{ stats.totalEssences }}</span>
        <span class="stat-label">精华帖总数</span>
      </div>
      <div class="stat-card">
        <span class="stat-value">{{ stats.totalAuthors }}</span>
        <span class="stat-label">精华作者数</span>
      </div>
      <div class="stat-card">
    ...
    <div class="filter-bar">
      <input v-model="searchKeyword" type="text" placeholder="搜索标题或作者..." class="search-input" @keyup.enter="searchEssences" />
      <button @click="searchEssences" class="btn-search">搜索</button>
    </div>

    <div v-if="loading" class="loading">加载中...</div>
    <div v-else-if="error" class="error">{{ error }}</div>
    <div v-else-if="essences.length === 0" class="empty">暂无精华帖</div>
    <div v-else class="essence-table">
      <table>
        <thead>
          <tr>
            <th>ID</th>
        ...
        <tbody>
          <tr v-for="post in essences" :key="post.id">
            <td>{{ post.id }}</td>
            ...
            <td>{{ formatDate(post.essenceTime) }}</td>
            <td class="actions">
              <button @click="viewPost(post)" class="btn-action btn-view">查看</button>
              <button @click="removeEssence(post.id)" class="btn-action btn-remove">取消精华</button>
              <button @click="setTop(post.id)" class="btn-action btn-top">置顶</button>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body" v-if="selectedPost">
          <div class="detail-grid">
            ...
            <div class="detail-item">
              <label>精华时间</label>
              <span>{{ formatDate(selectedPost.essenceTime) }}</span>
            </div>
            <div class="detail-item">
              <label>发布时间</label>
...
const loadEssences = async (page = 0) => {
  loading.value = true
  error.value = ''
  try {
    ...
    const response = await axios.get('/api/admin/essences', {
      params,
      headers: { Authorization: `Bearer ${token}` }
    })
    if (response.data.content) {
      essences.value = response.data.content
...
const loadStats = async () => {
  try {
    const response = await axios.get('/api/admin/essence-stats', {
      headers: { Authorization: `Bearer ${token}` }
    })
...
const removeEssence = async (id) => {
  if (!confirm('确定要取消这篇帖子的精华状态吗？')) return
  try {
    await axios.delete(`/api/admin/posts/${id}/essence`, {
      headers: { Authorization: `Bearer ${token}` }
    })
    alert('取消精华成功')
    loadEssences(currentPage.value)
    loadStats()
  } catch (e) {
    alert('操作失败')
  }
...
const prevPage = () => {
  if (currentPage.value > 0) {
    loadEssences(currentPage.value - 1)
  }
}
